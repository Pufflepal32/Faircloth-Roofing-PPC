...
exports.handler = async (event) => {
  ...
  try {
    ...
    params.append("RC_intake[note][source]", data.source || "Gutter PPC Landing Page");

    // CAMPAIGN ATTRIBUTION DATA
    params.append("RC_intake[note][utm_source]", data.utm_source || "direct");
    params.append("RC_intake[note][utm_medium]", data.utm_medium || "none");
    params.append("RC_intake[note][utm_campaign]", data.utm_campaign || "none");

    if (data.utm_term) {
      params.append("RC_intake[note][utm_term]", data.utm_term);
    }
    if (data.utm_content) {
      params.append("RC_intake[note][utm_content]", data.utm_content);
    }
    if (data.gclid) {
      params.append("RC_intake[note][gclid]", data.gclid);
    }

    params.append("RC_intake[note][landing_page]", data.landing_page || "unknown");
    params.append("RC_intake[note][referrer]", data.referrer || "direct");
    ...
    const attributionSummary = [
      ...
      "Page: " + (data.landing_page || "unknown"),
    ].join(" | ");
    params.append("RC_intake[note][attribution]", attributionSummary);
